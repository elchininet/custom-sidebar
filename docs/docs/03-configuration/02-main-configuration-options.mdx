...
| analytics                          | Boolean            | no       | If this value is true, every click on the sidebar will create a [Logbook](https://www.home-assistant.io/integrations/logbook/) entry with the item that has been clicked and using the domain `person` and the entity relative to the user logged-in |
