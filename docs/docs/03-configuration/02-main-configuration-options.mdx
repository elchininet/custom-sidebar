...
* When the `width` property (or one of its properties) is a number, the width will be set in pixels. When it (or one of its properties) is a string, it should have the proper format, this is an integer followed by one of these units: `ex`, `px`, `rem`, `vh`, `vw`, `vmax`, `vmin`, or `%`. `extended` mode is when the sidebar is always visible (typically in desktop devices) and `hidden` mode is when the sidebar is hidden and it needs to be shown clicking on the header's top-right button (typically in mobile devices).
