...
* Take into account that if you use the `order` option, all the items placed in the bottom will be moved to the top by default. If you want to have some items in the bottom after being specified an `order` option, you need to add them to the `order` option and specify their `bottom` property on `true`.
