...
| bottom                    | Boolean | no        | Setting this property to `true` will group the item with the bottom items (Configuration, Notifications, etc). If you don't seet this property or set it as `false`, the items will be placed in the top items |
