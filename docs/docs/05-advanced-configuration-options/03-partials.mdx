...
import PartialsWithJavaScriptTemplatesExample from '@site/docs/_partials/05-advanced-configuration-options/04-partials/_with-javascript-templates.mdx';
import PartialsWithJinjaTemplatesExample from '@site/docs/_partials/05-advanced-configuration-options/04-partials/_with-jinja-templates.mdx';

# Partials

Partials are fragments of code that can be included in your templates. When a `@partial something` instruction is parsed, it will get the content of the partial named `something` and it will replace altogether the `@partial something` instruction by this content.
